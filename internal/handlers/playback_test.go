...
func TestPlaybackHandler_Play_MultipleUsersWithSameTag(t *testing.T) {
	// Create test database
	testDB, err := db.New(":memory:")
	if err != nil {
		t.Fatalf("Failed to create test database: %v", err)
	...
	user1 := models.NewUser("user1", "111111", "test-auth-token-1")
	userID1, err := testDB.CreateUser(user1)
	if err != nil {
		t.Fatalf("Failed to create user1: %v", err)
	}

	user2 := models.NewUser("user2", "222222", "test-auth-token-2")
	userID2, err := testDB.CreateUser(user2)
	if err != nil {
	...
	// Should return the most recent mapping (user2's "Movie B")
	if mediaTitle, ok := response["media_title"].(string); !ok || mediaTitle != "Movie B" {
		t.Errorf("media_title = %v, want Movie B (most recent mapping)", response["media_title"])
	}

	if mediaID, ok := response["media_id"].(string); !ok || mediaID != "22222" {
		t.Errorf("media_id = %v, want 22222 (most recent mapping)", response["media_id"])
	}
}
